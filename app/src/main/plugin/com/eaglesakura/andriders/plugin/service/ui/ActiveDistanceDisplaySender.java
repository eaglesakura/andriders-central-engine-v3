 ...
 */
public class ActiveDistanceDisplaySender extends DisplayDataSender {
    public static final String DISPLAY_ID = "DISTANCE_TODAY_SESSION_ACTIVE";

    private Float mTodayDistanceKm;

    private Float mSessionDistanceKm;

    public ActiveDistanceDisplaySender(@NonNull PluginConnection session) {
        super(session);
    }

    public ActiveDistanceDisplaySender bind() {
    ...
    private CentralDataHandler mDataHandler = new CentralDataHandler() {
        ...
        public void onReceived(RawCentralData newData) {
            mTodayDistanceKm = newData.today.activeDistanceKm;
            mSessionDistanceKm = newData.session.activeDistanceKm;
        }
    };

    public static DisplayKey newInformation(Context context) {
        DisplayKey result = new DisplayKey(context, DISPLAY_ID);
        result.setTitle(context.getString(R.string.Title_Display_ActiveDistance));
        result.setSummary(context.getString(R.string.Message_Display_ActiveDistanceSummary));
        return result;
    }
}
